...
$id = $_GET['id'] ?? '';
$b = row('SELECT tb.*, t.name AS transfer_name, t.from_location, t.to_location, t.duration_min, t.vehicle_type, cu.name AS customer_name, cu.email AS customer_email, cu.phone AS customer_phone FROM transfer_bookings tb JOIN transfers t ON t.id = tb.transfer_id JOIN customers cu ON cu.id = tb.customer_id WHERE tb.id = ?', [$id]);
if (!$b) { flash('Prenotazione non trovata', 'error'); redirect('/admin/transfer-bookings.php'); }

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrfCheck($_POST['csrf'] ?? null);
    $action = $_POST['action'] ?? '';
    if ($action === 'status') {
        q('UPDATE transfer_bookings SET status = ? WHERE id = ?', [$_POST['status'], $b['id']]);
        flash('Stato aggiornato');
    }
    if ($action === 'paid') {
        q('UPDATE transfer_bookings SET paid = ? WHERE id = ?', [(float)$_POST['paid'], $b['id']]);
        flash('Importo pagato aggiornato');
    }
    if ($action === 'delete') {
        q('DELETE FROM transfer_bookings WHERE id = ?', [$b['id']]);
        flash('Prenotazione eliminata');
        redirect('/admin/transfer-bookings.php');
    }
    redirect('/admin/transfer-booking.php?id=' . $b['id']);
}

$waN = preg_replace('/\D/', '', $b['customer_phone']);
$waLink = $waN ? 'https://wa.me/' . $waN : null;

$title = 'Transfer ' . $b['code'];
$_subtitle = 'Prenotazioni transfer';
...
<div class="space-y-5">
  <div class="flex items-end justify-between flex-wrap gap-3">
    <div>
      <a href="/admin/transfer-bookings.php" class="text-sm text-ink-500 hover:text-brand-600 inline-flex items-center gap-1"><i data-lucide="chevron-left" class="size-[14px]"></i> Tutte le prenotazioni transfer</a>
      <h1 class="font-display text-2xl sm:text-3xl font-bold mt-2"><?= e($b['code']) ?></h1>
      <div class="text-sm text-ink-500"><?= e($b['from_location']) ?> → <?= e($b['to_location']) ?> · <?= fmtDate($b['arrival_date']) ?> <?= e($b['arrival_time'] ?: '') ?></div>
    </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    <div class="lg:col-span-2 space-y-5">
      ...
      <div class="card p-5">
        <h3 class="font-display font-bold mb-3">Dettagli viaggio</h3>
        <dl class="grid grid-cols-2 gap-y-2 text-sm">
          <dt class="text-ink-500">Tratta</dt><dd class="font-medium"><?= e($b['transfer_name']) ?></dd>
          <dt class="text-ink-500">Veicolo</dt><dd class="font-medium"><?= e(t('transfer.veh.' . $b['vehicle_type'])) ?></dd>
          <dt class="text-ink-500">Data arrivo</dt><dd class="font-medium"><?= fmtDate($b['arrival_date']) ?></dd>
          <dt class="text-ink-500">Ora arrivo</dt><dd class="font-medium"><?= e($b['arrival_time'] ?: '—') ?></dd>
          <dt class="text-ink-500">Volo</dt><dd class="font-mono"><?= e($b['flight_number'] ?: '—') ?></dd>
          <dt class="text-ink-500">Destinazione</dt><dd class="font-medium"><?= e($b['destination'] ?: '—') ?></dd>
          <dt class="text-ink-500">Passeggeri</dt><dd class="font-medium tabular-nums"><?= (int)$b['passengers'] ?></dd>
          <dt class="text-ink-500">Durata stimata</dt><dd class="font-medium"><?= (int)$b['duration_min'] ?> min</dd>
      ...
      <div class="card p-5">
        ...
        <div class="grid grid-cols-2 gap-3 text-sm mb-4">
          ...
          <div><div class="text-ink-500 text-xs">Pagato</div><div class="font-display font-bold"><?= fmtMoney((float)$b['paid']) ?></div></div>
        </div>
    ...
    <div class="space-y-5">
      <form method="post" class="card p-5 space-y-3">
        ...
        <select class="input" name="status">
          <?php foreach (['pending'=>'In attesa','confirmed'=>'Confermata','completed'=>'Conclusa','cancelled'=>'Annullata'] as $k => $v): ?>
